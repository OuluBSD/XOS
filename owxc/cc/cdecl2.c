...
local SYM_HANDLE FuncDecl( SYMPTR sym, stg_classes stg_class, decl_state *state )
{
    ...
    if( old_sym_handle == 0 ) {
        ...
        if( (old_sym.flags & SYM_FUNCTION) == 0 ) {
            ...
            // check lang flags to make sure no one saw an incompatible
			//   prototype; if previous prototype specified calling convention
			//   and later definition does not, propagate the convention from
			//   the prototype

            if ((sym->attrib & FLAG_LANGUAGES) &&
					!ChkCompatibleLanguage( sym->attrib, old_sym.attrib))
