...
static void mapsummary(void)
{
	...
	{
		sel = sgd->sd_select;
	    fprintf(mapfp, "    Segment #%-2d %-15s %04X", sgd->sd_num,
				sgd->sd_sym->sy_name + 1, sel);
	    mapnline();

		msd = sgd->sd_head;
		while (msd != NULL)
		{
			fprintf(mapfp, "      Msect #%-2d %-15s %04X:%08X %d", msd->md_num,
				msd->md_sym->sy_name + 1, sel, msd->md_addr, msd->md_lsize);
		    mapnline();
			psd = msd->md_head;
			while (psd != NULL)
			{
	            fprintf(mapfp, "        Psect   %-15s %04X:%08X %d",
						psd->pd_sym->sy_name + 1, sel, psd->pd_base,
						psd->pd_lsize);
			    mapnline();
				psd = psd->pd_next;
			}
...
// Function to output warning associated with a symbol
