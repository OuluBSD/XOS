...
long XOSFNC xwsbitmapscalesmoothlow(
	XWSDRW *drw,
	ushort *datad,
    long    wd,
    long    hd,
    ...
    long    size)
{
    ...
    memset(datad, 0, 2 * wd * hd);		// Clear destination
	...
	for (i = 0; i < hd; i++)
	{
		...
		{
			xstart = scol[j];
			pixelv.red = pixelv.green = pixelv.blue = 0;
			for (m = 0; m < size; m++)
			{
				ppixel = lines + m * ws + xstart;
				for (n = 0; n < size; n++)
				{
					expcolor(&pixel0, (uchar *)(ppixel + n));
					pixelv.red += pixel0.red;
					pixelv.green += pixel0.green;
					pixelv.blue += pixel0.blue;
				}
			}
			pixelv.red = (long)(((float)(pixelv.red)) * norm);
			pixelv.green = (long)(((float)(pixelv.green)) * norm);
			pixelv.blue = (long)(((float)(pixelv.blue)) * norm);
			comcolor(&pixelv, (uchar *)(lined + j));
			if (xws_SchedReq > 0)
				fbrCtlRelease();
		}
	}
