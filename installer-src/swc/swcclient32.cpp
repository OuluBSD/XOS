...
/* static */ void swcClient32::readyinpavlthd(
	MSG  *msg,
	void *pntr)

{
	((swcClient32 *)pntr)->readyinpavl(msg);
}


void swcClient32::readyinpavl(
	MSG *msg)

{
	void (*functmp)(swcClient32 *wnw, uchar *bufr, int len, void *arg);
...
/* static */ void swcClient32::readyoutdonthd(
	MSG  *msg,
	void *pntr)

{
	((swcClient32 *)pntr)->readyoutdon(msg);
}


void swcClient32::readyoutdon(
	MSG *msg)
...
//************************************************************
// Function: swcClient32 - Constructor for the swcClient32 class
// Returned: Nothing
//************************************************************
...

{
	...
	if (!clientreg)
	{
		msginpavl = swcRespRegFunc(swcClient32::readyinpavlthd, this);
		msgoutdon = swcRespRegFunc(swcClient32::readyoutdonthd, this);
		clientreg = true;
	}
...
void swcClient32::openconnection(
	char  *netaddr,
	long   port,
...
void swcClient32::closeconnection(
	void      (*donefuncarg)(swcClient32 *wnw, long result, void *arg),
	void      *argarg)
...
void swcClient32::acceptreceive(
	void (*rcvfuncarg)(swcClient32 *wnw, uchar *bufr, int len, void *arg),
	void  *rcvargarg)
...
uint __stdcall swcClient32::tcpinthread(
	void *arg)
...
///			__asm int 3;
		...
		if ((rtn = getdata(((swcClient32 *)arg)->soc,
				((swcClient32 *)arg)->rcvbfr, 6)) < 0)
		{
			if (rtn == -1)
			...
			((swcClient32 *)arg)->inputfin = true;

///			__asm int 3;

			return (1);					// Terminate the input thread
		}
		size = sl(*(long *)(((swcClient32 *)arg)->rcvbfr));
...
///			__asm int 3;
		...
		{
			if ((rtn = getdata(((swcClient32 *)arg)->soc,
					((swcClient32 *)arg)->rcvbfr + 6, len)) < 0)
			{
				if (rtn == -1)
					rtn = xlatewinsocerror(WSAGetLastError());
				if (!(((swcClient32 *)arg)->closed))
...
///				__asm int 3;
		...
		if (crypt)
		{
			qdesDecBlock(((swcClient32 *)arg)->rcvbfr + 4, len + 2,  
					&(((swcClient32 *)arg)->instate)); // Decrypt the message
			size = len - 8 + (((((swcClient32 *)arg)->rcvbfr)[len + 5]) & 0x07);
		}
...
void swcClient32::ignorereceive(
	swcClient32 *wnw,
	uchar      *bufr,
	int         len,
	void       *arg)
...
uint __stdcall swcClient32::tcpoutthread(
	void *arg)

{
	...
	((swcClient32 *)arg)->startdone = TRUE;
	SetEvent(((swcClient32 *)arg)->starthndl);

	while (GetMessage(&msg, NULL, 0, 0))
	{
		putinring(2, msg.message, msg.wParam);
		if (((swcClient32 *)arg)->closed) // Is connection closed now?
		{
