...
{	"NOERRNo error indicated",								//  0
	...
	"BDPFXBad prefixed value",								// -263
	"REC2LRecord is too long",								// -264
	"REC2SRecord is too short",								// -265
	"INCBLIncorrect block length",							// -266
	"BDPRHBad parent handle",								// -267
	"BDWNHBad window handle",								// -268
	"NRMTEError reported by remote system",					// -269
	"NOCONNo console device is available",					// -270
	"NPS2LNetwork packet size is too large",				// -271
	"NKA2SNetwork keep-alive time is too small",			// -272
	"TEMP Resource temporarily unavailable",				// -273
	"FIL2LFile is too long",								// -274
	"INTRPFunction interrupted",							// -275
	"TMLNKToo many links",									// -276
	"BDLNKBad link",										// -277
	"NOLCKNo lock available",								// -278
	"ARG2LArgument is too long",							// -279
	"USBTAToo many USB device addresses in use",			// -280
	"USBTHToo many USB hubs",								// -281
	"USBTLUSB descriptor is too long",						// -282
	"USBDFUSB descriptor format error",						// -283
	"USBBCBad USB device configuration",					// -284
	"USBSTUSB device stalled",								// -285
	"USBBDUSB babble detected",								// -286
	"USBBEUSB bus error",									// -287
	"USBBSUSB bit stuff error",								// -288
	"USBCNUSB client not found",							// -289
	"USBNRUSB device did not respond",						// -290
	"CFGERDevice configuration is invalid",					// -291
	"CODERCodec error",										// -292
	"BFRERInvalid buffer address or size",					// -293
	"NOARGRequired argument is missing",					// -294
	"UNSPLUnsupported printer language",					// -295
	"PMDNAPrinter mode not available",						// -296
	"SRBRKSerial line break detected",						// -297
	"DK2LGDisk is too large",								// -298
	"IVMBRInvalid Master Boot Record",						// -299
	"BDFNTBad font file",									// -300
	"LGDINAleary logged in",								// -301
	"INCONCurrent value is inconsistant",					// -302
	"NAUNRNetwork is unreachable",							// -303
	"NACONAlready connected"								// -304
};
...

{
	...
	if (code > 0 || code <= (-(int)(sizeof(msgtbl)/sizeof(char *))) ||
		...
		sprintf(test, (code >= 0) ? "{%05.5d}%s" : "{%04.4d}%s", code,
