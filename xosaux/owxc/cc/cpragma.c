...
hw_reg_set AsmRegs = HW_D(HW_FULL);

void PragAsmRegs(void)
{
	AsmRegs = PragRegList();
	return;
}
	...
	int type)
{
    ...
    if (CurToken == T_LEFT_PAREN)
	{
		do
		{
			...
			if (CurToken != T_ID)
				break;
			exim = (EXIM *)CMemAlloc(offsetof(EXIM, name) + 1 + TokenLen);
			exim->next = NULL;
			exim->ctype = 0x80;
			exim->cclass = 0xA0;
			exim->stype = type;
...
void PragExport(void)
{
	geteximitems(2);
}


void PragImport(void)
{
	geteximitems(1);
}
...
/******************/
{
    ...
    if( CompFlags.cpp_output ) {                    /* 29-sep-90 */
        ...
        } else if( PragRecog( "asm_regs" ) ) {
            PragAsmRegs();

        } else if( PragRecog( "asm_regs" ) ) {
            PragAsmRegs();

        } else if( PragRecog( "import" ) ) {
            PragImport();

        } else if( PragRecog( "export" ) ) {
            PragExport();
