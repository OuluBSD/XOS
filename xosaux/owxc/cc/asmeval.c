...
/*********************************************************************************/
{
    ...
    case T_STRING:
		...
		if ((Toggles & TOGGLE_TEXT_REVERSE) && (new->value & 0xFFFFFF00) != 0)
		{
			if ((new->value & 0xFFFF0000) == 0)
				new->value = ((new->value & 0x00FF) << 8) |
						((new->value & 0xFF00) >> 8);
			else if ((new->value & 0xFF000000) == 0)
				new->value = ((new->value & 0x0000FF) << 16) |
					(new->value & 0x00FF00) | ((new->value & 0xFF0000) >> 16);
			else
				new->value = ((new->value & 0x000000FF) << 24) |
						((new->value & 0x0000FF00) << 8) |
						((new->value & 0x00FF0000) >> 8) |
						((new->value & 0xFF000000) >> 24);
		}
        break;
